...
\begin{remark}
    \hfill
    \begin{enumerate}
        \item Si no existe $\alpha \geq 0$ tal que $M_\infty(s, f) = C(e^{s^\alpha})$, diremos que $\rho(f) = \infty$.
...
\begin{theorem}[Teorema del álgebra del orden de crecimiento]
    ...
    Entonces
    \begin{enumerate}
        \item $p(cf) = \rho(f)$, si $c \in \mathbb{C} \setminus \{0\}$.
...
\begin{theorem}[Factorización de Hadamard]
    ...
    Entonces existe un polinomio $g$ con $grad(p) \leq \rho(f)$, satisfaciendo que
    $$f(z) = e^{g(z)}z^N \prod_{n=1}^\infty E_p\left(\frac{z}{a_n}\right)$$
    $g$ es única salvo constantes $2\pi i$ y $\rho(f) = \max\{grad(p), \sigma\}$.
\end{theorem}
